export default function (m, conn) {
	try {
		const defaultUser = {
			...
			health: 100,
			level: 1,
			limit: (global.DEFAULT_USER_LIMIT !== undefined && !isNaN(global.DEFAULT_USER_LIMIT)) ? parseInt(global.DEFAULT_USER_LIMIT) : 20,
			age: -1,
			regTime: -1,
			afk: -1,
			afkReason: '',
		...
		for (const jid of botJids) {
			if (!global.db.data.settings[jid]) {
				...
				global.db.data.settings[jid] = { ...defaultSettings, ...global.db.data.settings[jid] };
			}
		}
