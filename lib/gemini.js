...
export function isValidReply(txt) {
 ...
 */
export async function askGeminiWeb(prompt, systemPrompt = '', timeoutMs = 12000) {
...
/**
 * 2. Gemini Cloud Proxy (Fast Dedicated Endpoint)
 */
export async function askGeminiProxy(prompt, systemPrompt = '', timeoutMs = 8000) {
	const ctrl = new AbortController();
	const tid = setTimeout(() => ctrl.abort(), timeoutMs);
	try {
		const url =
		...
		throw new Error('Invalid Gemini Proxy response');
	} finally {
		clearTimeout(tid);
	}
}
...
/**
 * 3. Gemini Official Generative Language API with Key & Model Rotation
 */
export async function askGeminiOfficial(prompt, systemPrompt = '', history = [], timeoutMs = 8000) {
...
/**
 * 4. DuckDuckGo DuckChat (GPT-4o-mini, Fast Backup)
 */
export async function askDuckDuckGo(prompt, systemPrompt = '', timeoutMs = 7000) {
	const ctrl = new AbortController();
	const tid = setTimeout(() => ctrl.abort(), timeoutMs);
	try {
...
/**
 * 5. Airforce API (GPT-4o-mini)
 */
export async function askAirforce(prompt, systemPrompt = '', history = [], timeoutMs = 6000) {
	const ctrl = new AbortController();
	const tid = setTimeout(() => ctrl.abort(), timeoutMs);
	try {
 ...
 * 6. Nowtech AI
 */
export async function askNowtech(prompt, systemPrompt = '', timeoutMs = 7000) {
	const ctrl = new AbortController();
	const tid = setTimeout(() => ctrl.abort(), timeoutMs);
	try {
...
/**
 * 7. Pollinations AI POST
 */
export async function askPollinations(prompt, systemPrompt = '', history = [], timeoutMs = 8000) {
	const ctrl = new AbortController();
	const tid = setTimeout(() => ctrl.abort(), timeoutMs);
	try {
 ...
 */
export async function getSmartAIReply(prompt, options = {}) {
	const { systemPrompt = '', history = [] } = options;

	// Primary Race: Gemini Web + Gemini Proxy + Gemini Official + DuckDuckGo
	try {
		const result = await Promise.any([
			...
			askGeminiProxy(prompt, systemPrompt, 7000).then((r) => {
				console.log('🤖 [AI Engine] Winner: Gemini Cloud Proxy');
				return r;
			}),
