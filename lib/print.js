...
// Prevent MaxListenersExceededWarning from watchFile calls during worker reconnects
...
`.trim(),
		...
		sender,
		m ? m.exp : '?',
		user ? '|' + (user.exp || 0) + '|' + (user.limit || 0) + '|' + (user.level || 1) : '',
		m.chat + (chat ? ' ~' + chat : ''),
		m.mtype
			? m.mtype
					.replace(/message$/i, '')
	...
	if (m.messageStubParameters && Array.isArray(m.messageStubParameters)) {
		const formattedStubs = m.messageStubParameters
			.map((jid) => {
				if (typeof jid !== 'string') return '';
				jid = conn.decodeJid(jid);
				const cleanNum = (jid || '').split('@')[0].replace(/[^0-9]/g, '');
				const phoneStr = cleanNum ? (PhoneNumber('+' + cleanNum).getNumber('international') || ('+' + cleanNum)) : jid;
				let name = conn.getName(jid);
				return chalk.gray(phoneStr + (name ? ' ~' + name : ''));
			})
			.filter(Boolean)
			.join(', ');
		if (formattedStubs) console.log(formattedStubs);
	}
