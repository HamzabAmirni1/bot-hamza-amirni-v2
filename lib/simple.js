	...
	let sock = Object.defineProperties(conn, {
		...
		sendMessage: {
			value(jid, content, options = {}) {
				return OrigMsg(
						...
						annotations: [
							{
								embeddedContent: {
									embeddedMusic: {
										...
										artistAttribution: 'https://github.com/HamzabAmirni1/bot-hamza-amirni-v2',
									},
								},
								embeddedAction: true,
							},
					...
					{
						...options,
						userJid: options.userJid || conn.user?.jid || conn.decodeJid(conn.user?.id) || '',
		...
		sendButton: {
			async value(jid, content = {}, options = {}) {
				const bodyText = content.body || content.text || content.caption || content.content || '';
				...
				try {
					...
					if (mime) {
						const media = await prepareWAMessageMedia({ [mime]: content[mime] }, { upload: conn.waUploadToServer }).catch(() => null);
						if (media) {
							header = {
					...
					const targetJid = options.quoted?.sender || (jid && jid.endsWith('@s.whatsapp.net') ? jid : '');
					const userLang = (targetJid && global.db?.data?.users?.[targetJid]?.language) || 'darija';

					let rawButtons = content.buttons || [];
					rawButtons = rawButtons.map(btn => {
						try {
							const params = typeof btn.buttonParamsJson === 'string' ? JSON.parse(btn.buttonParamsJson) : (btn.buttonParamsJson || {});
							let txt = params.display_text || '';
							let id = params.id || '';
							let url = params.url || '';

							if (txt.includes('Channel') || txt.includes('قناة') || url.includes('channel')) {
								params.display_text = userLang === 'english' ? '📢 WhatsApp Channel' : '📢 قناة الواتساب';
							} else if ((txt.includes('Owner') || txt.includes('المطور') || txt.includes('مالك')) && !url && (id === '.owner' || id === 'owner' || id.startsWith('.owner'))) {
								params.display_text = userLang === 'english' ? '👑 Owner' : userLang === 'arabic' ? '👑 المطور' : '👑 مالك البوت';
							} else if ((id === '.lang' || id === 'lang') && !id.includes(' ')) {
								params.display_text = userLang === 'english' ? '🌐 Change Language' : '🌐 تغيير اللغة';
							}
					...
					const msg = generateWAMessageFromContent(
						jid,
						{
							interactiveMessage: {
								...
								footer: { text: String(footerText) },
								nativeFlowMessage: {
									buttons: rawButtons,
								},
							},
						},
						{ ...options, userJid: options.userJid || conn.user?.jid || conn.decodeJid(conn.user?.id) || '' }
					);
					...
					console.error('[sendButton Fallback]:', err?.message || err);
					// Fallback to plain message send
					const txt = footerText ? `${bodyText}\n\n_${footerText}_` : bodyText;
					if (content.image) return await conn.sendMessage(jid, { image: content.image, caption: txt }, options).catch(() => {});
					if (content.video) return await conn.sendMessage(jid, { video: content.video, caption: txt }, options).catch(() => {});
					return await conn.sendMessage(jid, { text: txt }, options).catch(() => {});
				}
			},
		},
		sendContact: {
...
END:VCARD
		...
		reply: {
			/**
			 * Reply to a message — with standard buttons + plain text fallback
			 * @param {String} jid
			...
			async value(jid, text = '', quoted, options = {}) {
				...
				try {
					if (jid && typeof conn?.sendPresenceUpdate === 'function') {
						conn.sendPresenceUpdate('composing', jid).catch(() => {});
					}
				} catch (_) {}

				// ── Standard 4 buttons (Dynamic Language) ──────────────
				const userJid = quoted?.sender || (jid && jid.endsWith('@s.whatsapp.net') ? jid : '');
				const userLang = (userJid && global.db?.data?.users?.[userJid]?.language) || 'darija';

				let btnTextChannel = '📢 قناة الواتساب';
				let btnTextOwner = '👑 مالك البوت';
				let btnTextLang = '🌐 تغيير اللغة';

				if (userLang === 'english') {
					btnTextChannel = '📢 WhatsApp Channel';
					btnTextOwner = '👑 Owner';
					btnTextLang = '🌐 Change Language';
				} else if (userLang === 'arabic') {
					btnTextChannel = '📢 قناة الواتساب';
					btnTextOwner = '👑 المطور';
					btnTextLang = '🌐 تغيير اللغة';
				}
				...
				const _stdBtns = [
					{
						...
						buttonParamsJson: JSON.stringify({
							display_text: '📸 Instagram • حمزة اعمرني',
							url: global.instagram || 'https://www.instagram.com/hamza_amirni_01',
							merchant_url: global.instagram || 'https://www.instagram.com/hamza_amirni_01'
						})
					},
					{
						name: 'cta_url',
						buttonParamsJson: JSON.stringify({
							display_text: '📘 Facebook • Hamza Amirni',
							url: global.facebook || 'https://www.facebook.com/hamzaamirni01',
							merchant_url: global.facebook || 'https://www.facebook.com/hamzaamirni01'
						})
					},
					{
						name: 'cta_url',
					...
					},
					{
						name: 'cta_url',
						buttonParamsJson: JSON.stringify({
							display_text: '✈️ Telegram • Hamza Amirni',
							url: global.telegram || 'https://t.me/hamzaamirni',
							merchant_url: global.telegram || 'https://t.me/hamzaamirni'
						})
					},
					{
						name: 'quick_reply',
						buttonParamsJson: JSON.stringify({
							display_text: btnTextOwner,
							id: '.owner'
						})
					},
					{
						name: 'quick_reply',
						buttonParamsJson: JSON.stringify({
							display_text: btnTextLang,
							id: '.lang'
						})
					}
				];

				// Try with buttons first, fallback to plain text if it fails
				try {
					...
					return conn.sendMessage(
						...
						{
							text: String(text),
							mentions: options?.mentions || (conn.parseMention ? conn.parseMention(String(text)) : [])
						},
						{ quoted, ...options }
					);
				}
		...
		copyNForward: {
			...
			async value(jid, message, forwardingScore = true, options = {}) {
				...
				m = generateWAMessageFromContent(jid, m, { ...options, userJid: options.userJid || conn.user?.jid || conn.decodeJid(conn.user?.id) || '' });
