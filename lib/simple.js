		...
		sendMessage: {
			value(jid, content, options = {}) {
				if (content && typeof content === 'object') {
					if (!content.mentions && (content.text || content.caption)) {
						const autoMentions = conn.parseMention ? conn.parseMention(content.text || content.caption || '') : [];
						if (autoMentions && autoMentions.length) content.mentions = autoMentions;
					}
				}
				return OrigMsg(
					jid,
					content,
					{
						...
						useCachedGroupMetadata: options.useCachedGroupMetadata !== undefined ? options.useCachedGroupMetadata : true,
					}
				);
			},
		},
