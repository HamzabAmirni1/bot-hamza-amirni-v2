			...
			async value(jid, text = '', quoted, options) {
				...
				try {
					if (jid && conn?.sendPresenceUpdate) {
						// 1. Short read delay (human glances at message)
						const readDelay = Math.floor(150 + Math.random() * 150); // 150-300ms
						await new Promise(r => setTimeout(r, readDelay));

						// 2. Start "composing" (يكتب الآن... / typing...)
						await conn.sendPresenceUpdate('composing', jid).catch(() => {});

						// 3. Fast typing: 8ms/char + small jitter, max 1500ms total
						const textLen = String(text || '').length;
						const typingTime = Math.min(Math.max(textLen * 8 + Math.floor(Math.random() * 300), 400), 1500);
