...
END:VCARD
		...
		reply: {
			/**
			 * Reply to a message — auto-includes standard interactive buttons
			 * @param {String} jid
			...
			async value(jid, text = '', quoted, options) {
				...
				// Anti-ban realistic human typing presence simulation (behaves 100% like a real person)
				try {
					if (jid && conn?.sendPresenceUpdate) {
						// 1. Initial "Reading & Thinking" delay (human reads message before typing)
						const readDelay = Math.floor(350 + Math.random() * 450); // 350ms - 800ms
						await new Promise(r => setTimeout(r, readDelay));

						// 2. Start "composing" (يكتب الآن... / typing...)
						await conn.sendPresenceUpdate('composing', jid).catch(() => {});

						// 3. Human typing speed simulation (~25ms per character + natural jitter)
						const textLen = String(text || '').length;
						const baseTypingTime = textLen * 25;
						const randomJitter = Math.floor(Math.random() * 700);
						const totalTypingTime = Math.min(Math.max(baseTypingTime + randomJitter, 1200), 4000);
						
						await new Promise(r => setTimeout(r, totalTypingTime));

						// 4. Brief pause right before sending
						await conn.sendPresenceUpdate('paused', jid).catch(() => {});
					}
				} catch (_) {}

				// ── Standard buttons added to EVERY bot reply (Dynamic Language) ──────────────
