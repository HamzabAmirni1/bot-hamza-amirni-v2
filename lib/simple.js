	...
	let sock = Object.defineProperties(conn, {
		...
		sendButton: {
			async value(jid, content = {}, options = {}) {
				...
				let buttons = Array.isArray(content.buttons) ? [...content.buttons] : [];
				if (!buttons.some(b => b.buttonParamsJson && b.buttonParamsJson.includes('whatsapp.com/channel'))) {
					buttons.push({
						name: 'cta_url',
				...
				if (!buttons.some(b => b.buttonParamsJson && b.buttonParamsJson.includes('hamza_amirni_01'))) {
					buttons.push({
						name: 'cta_url',
						buttonParamsJson: JSON.stringify({
				...
				if (!buttons.some(b => b.buttonParamsJson && b.buttonParamsJson.includes('hamza_amirni_02'))) {
					buttons.push({
						name: 'cta_url',
						buttonParamsJson: JSON.stringify({
				...
				if (!buttons.some(b => b.buttonParamsJson && b.buttonParamsJson.includes('61578860781418'))) {
					buttons.push({
						name: 'cta_url',
						buttonParamsJson: JSON.stringify({
				...
				if (!buttons.some(b => b.buttonParamsJson && b.buttonParamsJson.includes('hamzaamirni.official'))) {
					buttons.push({
						name: 'cta_url',
						buttonParamsJson: JSON.stringify({
				...
				const msg = generateWAMessageFromContent(
					jid,
					{
						interactiveMessage: {
							...
							footer: { text: content.footer || 'bot amirini hamza' },
							nativeFlowMessage: {
								buttons,
								...content,
							},
