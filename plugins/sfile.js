...
let handler = async (m, { conn, text }) => {
	...
	if (!text) throw t(
		'Input search query or Sfile URL!\n\nExample:\n.sfile WhatsApp',
		'أدخل اسم الملف أو رابط Sfile!\n\n*مثال:*\n← .sfile WhatsApp',
		'كتب اسم الملف ولا رابط Sfile!\n\n*مثال:*\n← .sfile WhatsApp'
	);

	if (/https:\/\/sfile\.co\//i.test(text)) {
		let res = await sfile.download(text, true);
		if (!res) throw t('Unable to download file', 'تعذر تحميل الملف', 'ما قدرناش ننزلوا الملف');

		await m.reply(
			Object.entries(res.metadata)
				.map(([k, v]) => `*• ${k.capitalize()}:* ${v}`)
				.join('\n')
				.replaceAll('_', ' ') + '\n\n' + t('_Sending file..._', '_جاري إرسال الملف..._', '_كنصيفطو الملف..._')
		);
		...
		if (!res.length) throw t(`Query "${query}" not found`, `لم يتم العثور على "${query}"`, `مالقينا حتى نتيجة لـ "${query}"`);

		m.reply(
			res
				.map((v) => `*${t('Title', 'العنوان', 'العنوان')}:* ${v.title}\n*${t('Size', 'الحجم', 'الحجم')}:* ${v.size}\n*${t('Link', 'الرابط', 'الرابط')}:* ${v.link}`)
				.join('\n\n')
				.trim()
		);
	}
