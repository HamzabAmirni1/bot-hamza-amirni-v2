...
let handler = async (m, { conn, args }) => {
	...
	const promptMsg = lang === 'english'
		? '*Example:*\n← .ssweb https://instagram.com/hamza_amirni_01'
		: lang === 'arabic'
		? '*مثال:*\n← .ssweb https://instagram.com/hamza_amirni_01'
		: '*مثال:*\n← .ssweb https://instagram.com/hamza_amirni_01';
	try {
		if (!args[0]) throw promptMsg;
		await m.react('📸');
		let buff = await ssweb.capture(args[0]);
		conn.sendMessage(m.chat, { image: buff, caption: lang === 'english' ? '✅ Screenshot ready!' : lang === 'arabic' ? '✅ تم أخذ لقطة الشاشة!' : '✅ هاذي لقطة الشاشة ديالك! 🔥' }, { quoted: m });
		await m.react('✅');
	} catch (e) {
		await m.react('❌');
